...
Commands:
  archive     一键归档整门课程的作业、课程内容和课程回放 [aliases: ar]
...
如果希望一次性下载某门课程的资料，可以使用 `archive` 命令。它会按课程创建目录，并把作业附件、课程内容和课程回放分别保存到不同子目录中：

```bash
pku3b archive "投资学" -o ./downloads
```

`COURSE` 可以是课程 ID，也可以是课程标题关键字；如果匹配到多门课程，会提示你交互选择。默认只查找当前学期课程，查找历史学期课程时可以加上 `--all-term`。

默认会归档全部板块。你也可以通过 `--sections` 只选择其中一部分：

```bash
pku3b archive "投资学" -o ./downloads --sections video
pku3b archive "投资学" -o ./downloads --sections assignment,course-content
```
...
归档时会清洗课程目录名和文件名中的非法字符。已存在的文件默认跳过；如果需要覆盖重新下载，可以加 `--overwrite`。如果使用了未启用 `video-download` feature 的构建，请不要选择 `video` 或 `all`，否则程序会提示需要支持视频下载的构建。
...
- 📦 归档整门课程资料: `pku3b archive <课程名关键字> -o ./downloads`
- 📦 只归档作业和课程内容: `pku3b archive <课程名关键字> -o ./downloads --sections assignment,course-content`
