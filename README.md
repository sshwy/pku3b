...
Commands:
  ...
  document    获取课程文档/课件/下载文档附件 [aliases: doc]
  notice      获取课程公告/通知 [aliases: n]
...
- 📂 下载作业附件到指定目录: `pku3b a down <ID> -d <DIR>`（默认当前目录，不存在会自动创建）
...
- 📂 下载课程文档/课件附件到指定目录: `pku3b doc down <ID> -d <DIR>`（默认当前目录，不存在会自动创建）
...
- ⏯️ 下载课程回放到指定目录: `pku3b v down <ID> -o <DIR>`（默认当前目录；`<DIR>` 需已存在）
